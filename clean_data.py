...
def cap_employment_length(df: pd.DataFrame) -> pd.DataFrame:
    ...
    age of 16. Vectorized — do not use .apply()/lambda for this operation.
    """
    ...
    assert (df.loc[valid, "person_emp_length"] <= max_possible[valid]).all(), \
        "Employment length cap failed"
    print(f"Capped {n_invalid} impossible employment-length values")
    return df


def flag_and_fill_unemployed(df: pd.DataFrame) -> pd.DataFrame:
    """Flag originally-missing employment length before filling with 0."""
    ...
    print(f"Flagged {df['is_unemployed'].sum()} unemployed/missing-employment borrowers")
    return df


def impute_interest_rate(df: pd.DataFrame) -> pd.DataFrame:
    """Fill missing loan_int_rate with the median rate for that loan_grade."""
...
def standardize_categoricals(df: pd.DataFrame, cols=CATEGORICAL_COLS) -> pd.DataFrame:
    """Strip whitespace and uppercase all categorical columns."""
    for col in cols:
...
def run_validation(df: pd.DataFrame) -> None:
    """Final assertion block — fails loudly if any cleaning step regresses."""
...
def clean_pipeline(input_path: str, output_path: str) -> pd.DataFrame:
    ...
    df = flag_and_fill_unemployed(df)
