...
def cap_employment_length(df: pd.DataFrame) -> pd.DataFrame:
    ...
    n_invalid = (
        df["person_emp_length"] > max_possible
    ).sum()

    df["person_emp_length"] = np.minimum(
    ...
    assert (
        df.loc[valid, "person_emp_length"]
        <= max_possible[valid]
    ).all(), "Employment length cap failed"

    print(
        f"Capped {n_invalid} impossible "
...
def flag_and_fill_missing_employment(
    df: pd.DataFrame,
) -> pd.DataFrame:
    """
    ...
    assert (
        df["person_emp_length"].isnull().sum() == 0
    ), "Unfilled employment-length nulls remain"

    print(
        f"Flagged {df['emp_length_missing'].sum():,} borrowers "
        "with missing employment-length information"
    )
...
def impute_interest_rate(df: pd.DataFrame) -> pd.DataFrame:
    ...
    assert (
        df["loan_int_rate"].isnull().sum() == 0
    ), "Unfilled interest-rate nulls remain"
...
def cap_income_outliers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    q1, q3 = df["person_income"].quantile(
        [0.25, 0.75]
    )
    ...
    assert (
        df["person_income"].max() <= upper_bound
    ), "Income cap failed"

    print(
        f"Capped {n_outliers} income values "
        f"above ${upper_bound:,.0f}"
    )
...
def run_validation(df: pd.DataFrame) -> None:
    ...
    assert (
        ...
        .isin([0, 1])
        .all()
    ), "Invalid employment-missing flag"
